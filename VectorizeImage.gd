@tool
@icon('res://addons/VectorizerImagePlugin/blue-up-right-arrow.svg')
...
## Minimum color intensity required to consider a pixel an edge. Higher values detect only strong edges.
@export_range(0.0, 1.0, 0.01) var edge_threshold: float

@export_subgroup("Segment")
## Maximum color intensity to consider a pixel as non-edge. Lower values detects weaker edges.
@export_range(0.0, 1.0, 0.0001) var non_edge_threshold: float
#@export var specific_segment: bool
#@export var segment_nr: int
### If enabled, applies Godot built-in Delaunay triangulation within each segment instead of a polygon triangulation.
#@export var use_delaunay: bool

@export_subgroup("Edge and Segment")
## Minimum difference between colors required to detect a transition between segments.
@export_range(0.0, 2.0, 0.01) var edge_contrast_threshold: float
...
func in_same_direction( dir_x: int, dir_y: int, threshold: float, x: int, y: int, sobel_rows: Array, image_x: int, image_y: int, direction: Vector2 ):
	...
	if(temp_x >= 0 and temp_x < image_x and temp_y >= 0 and temp_y < image_y):
		...
		if( direction.dot(direction2) > edge_contrast_threshold ):
			return true
	return false
	...
	for i in range(sobel_rows.size()):
		...
		for j in range(sobel_rows[i].size()):
			...
			if(i > 0):
				...
				if( direction.dot(u_direction) > edge_contrast_threshold or (direction.length_squared() < non_edge_threshold and u_direction.length_squared() < non_edge_threshold)):
			...
			if(j > 0):
				...
				if( direction.dot(l_direction) > edge_contrast_threshold or (direction.length_squared() < non_edge_threshold and l_direction.length_squared() < non_edge_threshold)):
					# Merging
					if(updated and pixel_group[i-1][j] != pixel_group[i][j-1]):
						var u_color = sobel_rows[i-1][j] as Color
						var u_direction = Vector2(u_color.r - 0.5, u_color.g - 0.5)
						if( l_direction.dot(u_direction) > edge_contrast_threshold or (u_direction.length_squared() < non_edge_threshold and l_direction.length_squared() < non_edge_threshold)):
				...
				while(true):
					...
					if(from == 0 or from == 1): 
						...
						if(x > 0 and current_group == pixel_group[y][x-1]):
							...
							current_location = Vector2(x, y+1)
							from = 2
			elif(Vector2(j, i) not in dict[group] and count >= dot_spacing) and j % floor(dot_spacing/4) == 0:
				count = 0
...
func sobel_edge(image_x: int, image_y: int, rows: Array, sobel_rows: Array) -> Array:
	...
	for y in range(0, image_y):
		...
		for x in range(i, image_x):
			...
			elif(count >= dot_spacing and x % floor(dot_spacing/4) == 0):
				count = 0
				dots.push_back(Vector2(x, y))
			else:
				count += randi_range(1, 5)
	...
	print("Dots algorithm took: %.3f ms" % duration_ms_dots)
	...
	print("Dots algorithm took: %.3f ms" % duration_ms_tri)
	for k in range(0, triangles.size(), 3):
		var a = dots[triangles[k]]
		var b = dots[triangles[k + 1]]
		var c = dots[triangles[k + 2]]
...
func segmenting(image_x: int, image_y: int, dots_in_block: int, rows: Array, sobel_rows: Array) -> Array:
	...
	for i in range(segments.keys().size()-1):
		...
		triangles = Geometry2D.triangulate_delaunay(segment)
			...
			#triangles = Geometry2D.triangulate_delaunay(segment)
		#else:
			#triangles = Geometry2D.triangulate_polygon(segment)
		if(triangles.size() < 2):
			continue
		for k in range(0, triangles.size(), 3):
			...
			var center = Vector2((floor(a.x + b.x + c.x) / 3),  (floor(a.y + b.y + c.y) / 3))
...
func set_mesh_instance_mesh(new_value: bool):
	...
	if texture_image as Texture2D:
		...
		preview.size = sobel_image.get_size()
		preview.texture = ImageTexture.create_from_image(sobel_image)
